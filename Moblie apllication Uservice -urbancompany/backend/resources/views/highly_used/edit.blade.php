<x-app-layout>
    <nav class="container">
        <ol class="list-reset py-4 pl-4 rounded flex bg-grey-light text-grey">
            ...
            <li>/</li>
            <li class="px-2 text-gray-500">Update Block List</li>
        </ol>
    </nav>
    <div>
        <div class="max-w-10xl mx-auto py-2 sm:px-6 lg:px-8">
            <div class="mt-5 md:mt-0 md:col-span-2">
                <form method="post" action="{{ route('blocklist.update', $blocklist->id) }}">
                    @csrf
                    @method('PUT')
                    <div class="shadow overflow-hidden sm:rounded-md">
                        <div class="px-4 py-5 bg-white sm:p-6">
                            <div class="grid grid-cols-12 gap-6">
                                <div class="col-span-4 sm:col-span-4">
                                    <label for="phone" class="block font-medium text-xs text-gray-700">Phone<span
                                    ...
                                    <input type="number" name="phone" id="phone" maxlength="15"
                                           ...
                                           value="{{ old('phone', $blocklist->phone) }}"/>
                                    @error('phone')
                                    <p class="text-xs text-red-600">{{ $message }}</p>
                                    @enderror
                                </div>
                                <div class="col-span-4 sm:col-span-4">
                                    <label for="name" class="block font-medium text-xs text-gray-700">Name<span
                                            class="text-red-500">&#42;</span></label>
                                    <input type="text" name="name" id="name"
                                           class="bg-gray-200 appearance-none border-1 border-gray-200 rounded w-full py-2 px-4 text-gray-700 leading-tight focus:outline-none focus:bg-white"
                                           value="{{ old('name', $blocklist->name) }}"/>
                                    @error('name')
                                    <p class="text-xs text-red-600">{{ $message }}</p>
                                    @enderror
                                </div>
                                <div class="col-span-4 sm:col-span-4">
                                    <label for="remarks"
                                           ...
                                           class="bg-gray-200 appearance-none border-1 border-gray-200 rounded w-full py-2 px-4 text-gray-700 leading-tight focus:outline-none focus:bg-white"
                                           value="{{ old('remarks', $blocklist->remarks) }}">{{ $blocklist->remarks }}</textarea>
                                    @error('remarks')
                                    <p class="text-xs text-red-600">{{ $message }}</p>
                                    @enderror
                                </div>
                            ...
                            <div class="grid grid-cols-12 gap-6">
                                <div class="col-span-4 sm:col-span-4">
                                    ...
                                    <select id="status" name="status" autocomplete="status"
                                        ...
                                        <option value="0" {{ $blocklist->status == 0 ? 'selected' : '' }}>
                                            In-Active
                                        </option>
                                        <option value="1" {{ $blocklist->status == 1 ? 'selected' : '' }}>
                                            Active
                                        </option>
                                    </select>
                                    @error('status')
                        ...
                        <div class="px-4 py-5 bg-white sm:p-6 hidden">
                            ...
                            <input type="number" name="created_by" id="created_by"
                                   class="bg-gray-200 appearance-none border-1 border-gray-200 rounded w-full py-2 px-4 text-gray-700 leading-tight focus:outline-none focus:bg-white"
                                   value="{{ old('created_by', $blocklist->created_by) }}"/>
                            @error('created_by')
                            <p class="text-xs text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
